...
# @PACKAGE@-@VERSION@ (libgenetic-package) #
...
update_desktop_database() {
  ...
  if test -f "/usr/bin/gdk-pixbuf-query-loaders" ]; then
    gdk-pixbuf-query-loaders --update-cache &>/dev/null;
  fi
  if test -f "/usr/bin/kbuildsycoca5"; then
...
update_fc_cache() {
	...
	if test -f "/usr/bin/fc-cache"; then
		...
		if test $? -eq $true; then
      for font_dirs in $($FIND /usr/share/fonts -type d | $AWK -v ORS=" " '{print $0}'); do
		  	/usr/bin/fc-cache $font_dirs &>/dev/null;
      done
		fi
	fi;
}
